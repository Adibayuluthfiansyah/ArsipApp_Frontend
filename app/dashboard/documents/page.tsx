...
export default function DocumentsPage() {
  ...
  // FIX: ID adalah string (UUID)
  const [documentToDelete, setDocumentToDelete] = useState<string | null>(null);

  const fetchCategories = useCallback(async () => {
    try {
      // FIX: Menggunakan PaginatedResponse
  ...
  useEffect(() => {
    fetchDocuments();
  }, [fetchDocuments]); // Hapus search dan categoryFilter agar tidak memicu fetch ganda

  // Handler untuk memicu pencarian saat user berhenti mengetik
  useEffect(() => {
    const handler = setTimeout(() => {
      fetchDocuments(1); // Selalu reset ke halaman 1 saat filter berubah
    }, 500); // Debounce 500ms
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-0">
          {loading && documents.data.length === 0 ? ( // Tampilkan spinner hanya jika data kosong
          ...
          ) : !loading && documents.data.length === 0 ? ( // Tampilkan "Tidak ada data"
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t gap-3">
                <div className="text-sm text-muted-foreground">
                  Menampilkan {documents.from} - {documents.to} dari{" "}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => fetchDocuments(documents.current_page + 1)}
                    disabled={
                      documents.current_page === documents.last_page || loading
                    }
                  >
                    Next
                  </Button>
